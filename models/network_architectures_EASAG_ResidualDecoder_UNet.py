...
class UnetGridGatingSignal3(nn.Module):
    def __init__(self, in_planes, out_planes, props, kernel_size=(1,1,1), stride=(1,1,1)):
        print(f"Entering UnetGridGatingSignal3.__init__ with in_planes={in_planes}, out_planes={out_planes}, kernel_size={kernel_size}, stride={stride}")
        ...
        print(f"Updated props['conv_op_kwargs']['stride'] = {props['conv_op_kwargs']['stride']}")
        ...
        print(f"Constructed self.conv1: {self.conv1}")
        print("Exiting UnetGridGatingSignal3.__init__")

    def forward(self, x):
        # print(f"Entering UnetGridGatingSignal3.forward with input shape: {x.shape}")
        out = self.conv1(x)
        # print(f"Exiting UnetGridGatingSignal3.forward with output shape: {out.shape}")
        return out
...
class GridAttentionBlock3D(nn.Module):
    def __init__(self, in_channels, gating_channels, inter_channels=None, props=None, theta_kernel=(3, 3, 3),
        ...
        # print(f"Entering GridAttentionBlock3D.__init__ with in_channels={in_channels}, gating_channels={gating_channels}, inter_channels={inter_channels}, theta_kernel={theta_kernel}, theta_stride={theta_stride}")
        super(GridAttentionBlock3D, self).__init__()

        props['conv_op_kwargs']['stride'] = print(f"Set props['conv_op_kwargs']['stride'] to 1") or 1
        self.props = print(f"Stored props: {props}") or props

        if theta_stride is not None:
            kwargs_conv1 = deepcopy(props['conv_op_kwargs'])
            kwargs_conv1['stride'] = print(f"Applying theta_stride={theta_stride} to kwargs_conv1") or theta_stride
        else:
            kwargs_conv1 = props['conv_op_kwargs']
            print(f"No theta_stride override, using default conv_op_kwargs: {kwargs_conv1}")

        # Downsampling rate for the input featuremap # will be stride and kernel for conv applied on x
        self.theta_kernel = print(f"theta_kernel set to: {theta_kernel}") or theta_kernel
        self.theta_stride = print(f"theta_stride set to: {theta_stride}") or theta_stride

        # Number of channels (pixel dimensions)
        self.in_channels = print(f"in_channels: {in_channels}") or in_channels
        self.gating_channels = print(f"gating_channels: {gating_channels}") or gating_channels
        self.inter_channels = print(f"inter_channels before adjustment: {inter_channels}") or inter_channels

        if self.inter_channels is None:
            self.inter_channels = in_channels // 2
            # print(f"inter_channels was None, set to in_channels//2: {self.inter_channels}")
            if self.inter_channels == 0:
                self.inter_channels = 1
                print("inter_channels was 0, set to 1")

        # print(f"Final inter_channels: {self.inter_channels}")
...
###################################################################################################################
        ...
        # print(f"psi conv: {self.psi}")

        # Output transform
        # print("Initializing output transform W")
        ...
        # print(f"W transform: {self.W}")
        # print("Exiting GridAttentionBlock3D.__init__()")

    def forward(self, x, g):
        # print(f"Entering forward with x shape: {x.shape}, g shape: {g.shape}")
        output = self._concatenation(x, g)
        # print(f"Exiting forward with output shapes: {tuple(o.shape for o in output)}")
        return output

    def _concatenation(self, x, g):
        # print(f"Entering _concatenation with x shape: {x.shape}, g shape: {g.shape}")
        input_size = x.size()
        ...
        # print("Applying phi to g and interpolating to theta_x size")
        ...
        # print(f"phi_g shape after interpolate: {phi_g.shape}")

        # gate and input added and relu executed
        # print("Computing f = ReLU(theta_x + phi_g)")
        f = F.relu(theta_x + phi_g, inplace=True)
        # print(f"f shape: {f.shape}")

        #  psi^T * f -> (b, psi_i_c, t/s1, h/s2, w/s3)
        # sigmoid used to scale weights
        # print("Applying psi and sigmoid to f")
        sigm_psi_f = torch.sigmoid(self.psi(f))
        # print(f"sigm_psi_f shape: {sigm_psi_f.shape}")

        # upsample the attentions and multiply
        # upsample weights back to input size
        # print("Interpolating attention weights back to input size")
        ...
        # print(f"sigm_psi_f shape after interpolate: {sigm_psi_f.shape}")

        # print("Applying attention weights to input x")
        y = sigm_psi_f.expand_as(x) * x
        # print(f"y shape: {y.shape}")

        # print("Applying W transform to y")
        W_y = self.W(y)
        # print(f"W_y shape: {W_y.shape}")

        # print("Exiting _concatenation")
...
class ResidualSingleAttentionUNetDecoder(nn.Module):
    def __init__(self, previous, num_classes, num_blocks_per_stage=None, network_props=None, deep_supervision=False,
                 upscale_logits=False, block=BasicResidualBlock):
        
        super(ResidualSingleAttentionUNetDecoder, self).__init__()
        self.num_classes = num_classes
        self.deep_supervision = deep_supervision
        # We assume the bottleneck is part of the encoder, so we can start with upsample -> concat here
        previous_stages = previous.stages
        ...
        for i, s in enumerate(np.arange(num_stages)[::-1]):
            ...
            att_block = EdgeAwareMSAG(in_channels=features_skip, gating_channels=features_below, inter_channels=features_skip,
...
class ResidualDecoderSingleAttentionUNet(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_blocks_per_stage_encoder, feat_map_mul_on_downscale,
        ...
        super(ResidualDecoderSingleAttentionUNet, self).__init__()
        ...
        self.decoder = ResidualSingleAttentionUNetDecoder(self.encoder, num_classes, num_blocks_per_stage_decoder, props,
