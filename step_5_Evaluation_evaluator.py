...
sys.path.append(os.path.abspath('/home/ss_students/zywx/Label_vs_Prediction'))
sys.path.append(os.path.abspath('/home/ss_students/zywx/Label_vs_Prediction/batchgenerators'))
...
from all_helper_fn_n_class.evaluation_metrics import ConfusionMatrix, ALL_METRICS
from all_helper_fn_n_class.batchgenerators.utilities_file_and_folder_operations import save_json, subfiles, join
...
class Evaluator:
    ...
    def evaluate(self, test=None, reference=None, advanced=False, **metric_kwargs):
        ...
        eval_metrics = list(self.metrics)
        if advanced:
        ...
        if isinstance(self.labels, dict):
            for label, name in self.labels.items():
                ...
                if not hasattr(label, '__iterator'):
    ...
    def to_pandas(self):
        ...
        if isinstance(self.labels, dict):
            ...
            labels = self.labels
        result_metrics = sorted(self.result[list(self.result.keys())[0]].keys())
        df = pd.DataFrame(a, index=labels, columns=result_metrics)
        return df
...
class NiftiEvaluator(Evaluator):
    ...
    def set_test(self, test):
        ...
        if test is not None:
            ...
            array = sitk.GetArrayFromImage(self.test_nifti)
            super(NiftiEvaluator, self).set_test(array)
        else:
            self.test_nifti = None
    ...
    def set_reference(self, reference):
        ...
        if reference is not None:
            ...
            array = sitk.GetArrayFromImage(self.reference_nifti)
            super(NiftiEvaluator, self).set_reference(array)
        else:
            self.reference_nifti = None
    ...
    def evaluate(self, test=None, reference=None, voxel_spacing=None, **metric_kwargs):
        ...
        result = super(NiftiEvaluator, self).evaluate(test, reference, **metric_kwargs)
        return result
...
def run_evaluation(args):
    ...
    if evaluator.labels is None:
        ...
        print(f"Using existing labels: {evaluator.labels}")
...
def aggregate_scores(test_ref_pairs,
    ...
    :return:
    """
    print("Starting aggregate_scores")
...
def nnunet_evaluate_folder(
    ...
    """
    Evaluate a folder of predicted segmentations against references.

    Parameters:
    - reference_folder: path to folder containing ground-truth NIfTI segmentations.
    - prediction_folder: path to folder containing predicted NIfTI segmentations (filenames must match labels).
    - label_ids: list of int, label IDs to evaluate (e.g. [1, 2] for liver and tumor).

    Returns:
    The JSON-like evaluation metrics as returned by evaluate_folder().
    """
...
reference_folder = '/home/ss_students/zywx/Label_vs_Prediction/label'
prediction_folder = '/home/ss_students/zywx/Label_vs_Prediction/prediction'
