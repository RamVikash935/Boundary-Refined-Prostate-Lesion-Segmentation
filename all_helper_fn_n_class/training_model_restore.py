    ...
    if tr is None:
        for importer, modname, ispkg in pkgutil.iter_modules([folder]):
            if ispkg:
                ...
                tr = recursive_find_python_class(join(folder, modname), trainer_name, current_module=next_current_module)
            if tr is not None:
                break
...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None):
    ...
    tr = recursive_find_python_class(search_in, name,"all_helper_fn_n_class")
