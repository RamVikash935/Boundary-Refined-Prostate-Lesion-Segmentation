...
import numpy as np
from experiment_planning_experiment_planner_baseline_3DUNet_v21 import ExperimentPlanner3D_v21
from experiment_planning_common_utils import get_pool_and_conv_props
from paths import *
from all_helper_fn_n_class.network_architectures_LEASAG_Attention_ResidualDecoder_UNet import LearnableEASAGResidualDecoderAttentionUNet

class ExperimentPlanner_Larnable_EASAG_Attention_UNet(ExperimentPlanner3D_v21):
    """ Planner for the Fully GenericAttentionUNet 3D UNet"""
    def __init__(self, folder_with_cropped_data, preprocessed_output_folder):
        super(ExperimentPlanner_Larnable_EASAG_Attention_UNet, self).__init__(folder_with_cropped_data, preprocessed_output_folder)
        self.data_identifier = "nnUNetData_plans_v2.1"  # "nnUNetData_FabiansResUNet_v2.1"
        self.plans_fname = join(self.preprocessed_output_folder,
                                "nnUNetPlans_Larnable_EASAG_ResidualDecoder_AttentionUNet_v2.1_plans_3D.pkl")
    ...
    def get_properties_for_stage(self, current_spacing, original_spacing, original_shape, num_cases,
        ...
        blocks_per_stage_encoder = LearnableEASAGResidualDecoderAttentionUNet.default_blocks_per_stage_encoder[:len(pool_op_kernel_sizes)]
        blocks_per_stage_decoder = LearnableEASAGResidualDecoderAttentionUNet.default_blocks_per_stage_decoder[:len(pool_op_kernel_sizes) - 1]

        ref = LearnableEASAGResidualDecoderAttentionUNet.use_this_for_3D_configuration
        here = LearnableEASAGResidualDecoderAttentionUNet.compute_approx_vram_consumption(input_patch_size, self.unet_base_num_features,
        ...
        while here > ref:
            ...
            blocks_per_stage_encoder = LearnableEASAGResidualDecoderAttentionUNet.default_blocks_per_stage_encoder[:len(pool_op_kernel_sizes)]
            blocks_per_stage_decoder = LearnableEASAGResidualDecoderAttentionUNet.default_blocks_per_stage_decoder[:len(pool_op_kernel_sizes) - 1]
            here = LearnableEASAGResidualDecoderAttentionUNet.compute_approx_vram_consumption(new_shp, self.unet_base_num_features,
                                                               self.unet_max_num_filters, num_modalities, num_classes,
                                                               pool_op_kernel_sizes, blocks_per_stage_encoder,
                                                               blocks_per_stage_decoder, 2, self.unet_min_batch_size)
        input_patch_size = new_shp

        batch_size = LearnableEASAGResidualDecoderAttentionUNet.default_min_batch_size
        ...
        batch_size = min(1, min(batch_size, max_batch_size))
