...
import numpy as np
from batchgenerators.augmentations_utils import pad_nd_image
from utilities import no_op, to_cuda, maybe_to_torch
...
class NeuralNetwork(nn.Module):
    ...
    def get_device(self):
        ...
        print(f"Device of first parameter: {device}")
        if device.type == "cpu":
            print("Device type is CPU")
            result = "cpu"
        else:
            print(f"Device type is CUDA, index: {device.index}")
            result = device.index
            
        print(f"get_device() returning: {result}")
        return result

    def set_device(self, device):
        print(f"Entering set_device() with device: {device}")
        if device == "cpu":
            print("Setting model to CPU")
            self.cpu()
        else:
            print(f"Setting model to CUDA:{device}")
            self.cuda(device)
            
        print("set_device() complete")

    def forward(self, x):
        print(f"Entering forward() with input x: {x}")
...
class SegmentationNetwork(NeuralNetwork):
    def __init__(self):
        print("Initializing SegmentationNetwork")        
        super(NeuralNetwork, self).__init__()

        # if we have 5 pooling then our patch size must be divisible by 2**5
        self.input_shape_must_be_divisible_by = None  # for example in a 2d network that does 5 pool in x and 6 pool in y this would be (32, 64)
        print(f"Set input_shape_must_be_divisible_by: {self.input_shape_must_be_divisible_by}")
        
        # we need to know this because we need to know if we are a 2d or a 3d netowrk
        self.conv_op = None  # nn.Conv2d or nn.Conv3d
        print(f"Set conv_op: {self.conv_op}")
        ...
        print(f"Set num_classes: {self.num_classes}")        
        ...
        print(f"Set inference_apply_nonlin: {self.inference_apply_nonlin}")        

        # This is for saving a gaussian importance map for inference. It weights voxels higher that are closer to the
        # center. Prediction at the borders are often less accurate and are thus downweighted. Creating these Gaussians
        # can be expensive, so it makes sense to save and reuse them.
        self._gaussian_3d = self._patch_size_for_gaussian_3d = None
        print(f"Set _gaussian_3d & _patch_size_for_gaussian_3d: {self._gaussian_3d}, {self._patch_size_for_gaussian_3d}")
                
        self._gaussian_2d = self._patch_size_for_gaussian_2d = None
        print(f"Set _gaussian_2d & _patch_size_for_gaussian_2d: {self._gaussian_2d}, {self._patch_size_for_gaussian_2d}")        
    ...
    def predict_3D(self, x: np.ndarray, do_mirroring: bool, mirror_axes: Tuple[int, ...] = (0, 1, 2),
        ...
        :return:
        """
        print("Entered predict_3D")
        torch.cuda.empty_cache()

        print(f"Asserting step_size <= 1, actual: {step_size}")
        ...
        if pad_kwargs is None:
            print("pad_kwargs is None, setting default")
            pad_kwargs = {'constant_values': 0}
            
        print(f"pad_kwargs after init: {pad_kwargs}")

        # A very long time ago the mirror axes were (2, 3, 4) for a 3d network. This is just to intercept any old
        # code that uses this convention
        if len(mirror_axes):
            print(f"Checking mirror_axes {mirror_axes} for conv_op {self.conv_op}")
            if self.conv_op == nn.Conv2d:
                print("conv_op is Conv2d")
                if max(mirror_axes) > 1:
                    raise ValueError("mirror axes. duh")
            if self.conv_op == nn.Conv3d:
                print("conv_op is Conv3d")
    ...
    def predict_2D(self, x, do_mirroring: bool, mirror_axes: tuple = (0, 1, 2), use_sliding_window: bool = False,
        ...
        print(f"Checking conv_op for 2D: {self.conv_op}")
        if self.conv_op == nn.Conv3d:
        ...
        # code that uses this convention
        if len(mirror_axes):
            print(f"Checking mirror_axes {mirror_axes}")            
            if max(mirror_axes) > 1:
                print("Invalid mirror_axes for 2D, raising ValueError")
                raise ValueError("mirror axes. duh")

        if self.training:
            print('WARNING! Network is in train mode during inference. This may be intended, or not...')

        print(f"Input x shape: {getattr(x, 'shape', 'unknown')}")
        ...
        print(f"predict_2D returning result: {res}")
        return res
