...
def get_pool_and_conv_props_poolLateV2(patch_size, min_feature_map_size, max_numpool, spacing):
    ...
    :return:
    """
    print(f"[DEBUG] Called get_pool_and_conv_props_poolLateV2 with patch_size={patch_size}, min_feature_map_size={min_feature_map_size}, max_numpool={max_numpool}, spacing={spacing}")

    initial_spacing = deepcopy(spacing)
    print(f"[DEBUG] initial_spacing set to: {initial_spacing}")

    reach = max(initial_spacing)
    print(f"[DEBUG] reach (max initial_spacing): {reach}")

    dim = len(patch_size)
    print(f"[DEBUG] dimensionality (dim): {dim}")

    num_pool_per_axis = get_network_numpool(patch_size, max_numpool, min_feature_map_size)
    print(f"[DEBUG] num_pool_per_axis: {num_pool_per_axis}")

    net_num_pool_op_kernel_sizes = []
    print(f"[DEBUG] Initialized net_num_pool_op_kernel_sizes: {net_num_pool_op_kernel_sizes}")

    net_conv_kernel_sizes = []
    print(f"[DEBUG] Initialized net_conv_kernel_sizes: {net_conv_kernel_sizes}")

    net_numpool = max(num_pool_per_axis)
    print(f"[DEBUG] net_numpool (max of num_pool_per_axis): {net_numpool}")

    current_spacing = spacing
    print(f"[DEBUG] Starting current_spacing: {current_spacing}")

    for p in range(net_numpool):
        print(f"[DEBUG] Pooling iteration p={p}")

        reached = [current_spacing[i] / reach > 0.5 for i in range(dim)]
        print(f"[DEBUG] reached flags: {reached}")

        pool = [2 if num_pool_per_axis[i] + p >= net_numpool else 1 for i in range(dim)]
        print(f"[DEBUG] pool sizes for this iteration: {pool}")

        if all(reached):
            conv = [3] * dim
            print(f"[DEBUG] all(reached) is True, conv sizes: {conv}")
        else:
            conv = [3 if not reached[i] else 1 for i in range(dim)]
            print(f"[DEBUG] some reached False, conv sizes: {conv}")

        net_num_pool_op_kernel_sizes.append(pool)
        print(f"[DEBUG] Appended to net_num_pool_op_kernel_sizes: {net_num_pool_op_kernel_sizes}")

        net_conv_kernel_sizes.append(conv)
        print(f"[DEBUG] Appended to net_conv_kernel_sizes: {net_conv_kernel_sizes}")

        current_spacing = [i * j for i, j in zip(current_spacing, pool)]
        print(f"[DEBUG] Updated current_spacing: {current_spacing}")

    net_conv_kernel_sizes.append([3] * dim)
    print(f"[DEBUG] Appended final conv kernel size for bottleneck: {[3] * dim}")

    must_be_divisible_by = get_shape_must_be_divisible_by(num_pool_per_axis)
    print(f"[DEBUG] must_be_divisible_by: {must_be_divisible_by}")

    patch_size = pad_shape(patch_size, must_be_divisible_by)
    print(f"[DEBUG] Padded patch_size: {patch_size}")

    # return computed values
    print(f"[DEBUG] Returning num_pool_per_axis={num_pool_per_axis}, net_num_pool_op_kernel_sizes={net_num_pool_op_kernel_sizes}, net_conv_kernel_sizes={net_conv_kernel_sizes}, patch_size={patch_size}, must_be_divisible_by={must_be_divisible_by}")
