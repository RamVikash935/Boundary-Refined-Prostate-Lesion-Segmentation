...
from experiment_planning_common_utils import get_pool_and_conv_props_poolLateV2
from experiment_planning_utils import create_lists_from_splitted_dataset
from network_architectures_generic_UNet import Generic_UNet
from paths import *
from preprocessing_cropping import get_case_identifier_from_npz
from training_model_restore import recursive_find_python_class


class ExperimentPlanner(object):
    def __init__(self, folder_with_cropped_data, preprocessed_output_folder):
        # print(f"[DEBUG] Initializing ExperimentPlanner with folder_with_cropped_data={folder_with_cropped_data}, preprocessed_output_folder={preprocessed_output_folder}")
        self.folder_with_cropped_data = folder_with_cropped_data
        # print(f"[DEBUG] Set self.folder_with_cropped_data = {self.folder_with_cropped_data}")
        self.preprocessed_output_folder = preprocessed_output_folder
        # print(f"[DEBUG] Set self.preprocessed_output_folder = {self.preprocessed_output_folder}")

        self.list_of_cropped_npz_files = subfiles(self.folder_with_cropped_data, True, None, ".npz", True)
        # print(f"[DEBUG] Retrieved list_of_cropped_npz_files: {self.list_of_cropped_npz_files}")

        self.preprocessor_name = "GenericPreprocessor"
        # print(f"[DEBUG] Set self.preprocessor_name = {self.preprocessor_name}")
        ...
        # print(f"[DEBUG] Found dataset_properties.pkl in {self.folder_with_cropped_data}")

        self.dataset_properties = load_pickle(join(self.folder_with_cropped_data, "dataset_properties.pkl"))
        # print(f"[DEBUG] Loaded dataset_properties: keys = {list(self.dataset_properties.keys())}")

        self.plans_per_stage = OrderedDict()
        # print(f"[DEBUG] Initialized self.plans_per_stage = OrderedDict()")
        self.plans = OrderedDict()
        # print(f"[DEBUG] Initialized self.plans = OrderedDict()")

        self.plans_fname = join(self.preprocessed_output_folder, "nnUNetPlansfixed_plans_3D.pkl")
        print(f"[DEBUG] Set self.plans_fname = {self.plans_fname}")

        self.data_identifier = default_data_identifier
        print(f"[DEBUG] Set self.data_identifier = {self.data_identifier}")
        ...
        print(f"[DEBUG] Set self.unet_base_num_features = {self.unet_base_num_features}")
        self.unet_max_num_filters = 320
        print(f"[DEBUG] Set self.unet_max_num_filters = {self.unet_max_num_filters}")
        self.unet_max_numpool = 999
        print(f"[DEBUG] Set self.unet_max_numpool = {self.unet_max_numpool}")
        self.unet_min_batch_size = 2
        print(f"[DEBUG] Set self.unet_min_batch_size = {self.unet_min_batch_size}")
        self.unet_featuremap_min_edge_length = 4
        print(f"[DEBUG] Set self.unet_featuremap_min_edge_length = {self.unet_featuremap_min_edge_length}")

        self.target_spacing_percentile = 50
        print(f"[DEBUG] Set self.target_spacing_percentile = {self.target_spacing_percentile}")
        self.anisotropy_threshold = 3
        print(f"[DEBUG] Set self.anisotropy_threshold = {self.anisotropy_threshold}")
        self.how_much_of_a_patient_must_the_network_see_at_stage0 = 4
        print(f"[DEBUG] Set self.how_much_of_a_patient_must_the_network_see_at_stage0 = {self.how_much_of_a_patient_must_the_network_see_at_stage0}")
        self.batch_size_covers_max_percent_of_dataset = 0.05
        print(f"[DEBUG] Set self.batch_size_covers_max_percent_of_dataset = {self.batch_size_covers_max_percent_of_dataset}")

        self.conv_per_stage = 2
        print(f"[DEBUG] Set self.conv_per_stage = {self.conv_per_stage}")

    def get_target_spacing(self):
        # print(f"[DEBUG] get_target_spacing called")
        ...
        print(f"[DEBUG] Computed target spacing at {self.target_spacing_percentile}th percentile: {target}")
        return target


    def save_my_plans(self):
        print(f"[DEBUG] save_my_plans called, plans_fname = {self.plans_fname}")
        with open(self.plans_fname, 'wb') as f:
            print(f"[DEBUG] Opened {self.plans_fname} for writing")
            pickle.dump(self.plans, f)
            print(f"[DEBUG] Dumped self.plans to {self.plans_fname}")

    def load_my_plans(self):
        print(f"[DEBUG] load_my_plans called, plans_fname = {self.plans_fname}")
        self.plans = load_pickle(self.plans_fname)
        print(f"[DEBUG] Loaded plans: keys = {list(self.plans.keys())}")

        self.plans_per_stage = self.plans['plans_per_stage']
        print(f"[DEBUG] Set self.plans_per_stage = {self.plans_per_stage}")

        self.dataset_properties = self.plans['dataset_properties']
        print(f"[DEBUG] Set self.dataset_properties = {self.dataset_properties.keys()}")
    ...
    def get_properties_for_stage(self, current_spacing, original_spacing, original_shape, num_cases,
        ...
        :return:
        """    
        print(f"[DEBUG] get_properties_for_stage called with current_spacing={current_spacing}, original_spacing={original_spacing}, original_shape={original_shape}, num_cases={num_cases}, num_modalities={num_modalities}, num_classes={num_classes}")
        
        new_median_shape = np.round(original_spacing / current_spacing * original_shape).astype(int)
        print(f"[DEBUG] Computed new_median_shape: {new_median_shape}")
        
        dataset_num_voxels = np.prod(new_median_shape) * num_cases
        print(f"[DEBUG] Computed dataset_num_voxels: {dataset_num_voxels}")
        
        input_patch_size = 1 / np.array(current_spacing)
        print(f"[DEBUG] Initial input_patch_size (voxels per mm): {input_patch_size}")
        
        input_patch_size /= input_patch_size.mean()
        print(f"[DEBUG] Normalized input_patch_size: {input_patch_size}")
        
        input_patch_size *= 1 / min(input_patch_size) * 512
        input_patch_size = np.round(input_patch_size).astype(int)
        print(f"[DEBUG] Rescaled and rounded input_patch_size to isotropic 512mm box: {input_patch_size}")
        
        input_patch_size = [min(i, j) for i, j in zip(input_patch_size, new_median_shape)]
        print(f"[DEBUG] Clipped input_patch_size to new_median_shape: {input_patch_size}")
        
        network_num_pool_per_axis, pool_op_kernel_sizes, conv_kernel_sizes, new_shp, shape_must_be_divisible_by = get_pool_and_conv_props_poolLateV2(
            input_patch_size, self.unet_featuremap_min_edge_length, self.unet_max_numpool, current_spacing)
        print(f"[DEBUG] Computed pooling and convolution properties")
        
        ref = Generic_UNet.use_this_for_batch_size_computation_3D
        print(f"[DEBUG] Reference VRAM consumption: {ref}")
        ...
        print(f"[DEBUG] Current VRAM consumption estimate: {here}")
        
        while here > ref:
            print(f"[DEBUG] VRAM consumption {here} > {ref}, reducing patch size")
            
            axis_to_be_reduced = np.argsort(new_shp / new_median_shape)[-1]
            print(f"[DEBUG] Reducing axis {axis_to_be_reduced}")
            
            tmp = deepcopy(new_shp)
            tmp[axis_to_be_reduced] -= shape_must_be_divisible_by[axis_to_be_reduced]
            print(f"[DEBUG] Temporary new shape: {tmp}")
            
            _, _, _, _, shape_must_be_divisible_by_new = get_pool_and_conv_props_poolLateV2(
                tmp, self.unet_featuremap_min_edge_length, self.unet_max_numpool, current_spacing)
            new_shp[axis_to_be_reduced] -= shape_must_be_divisible_by_new[axis_to_be_reduced]
            print(f"[DEBUG] Updated new_shp: {new_shp}")
            
            network_num_pool_per_axis, pool_op_kernel_sizes, conv_kernel_sizes, new_shp, shape_must_be_divisible_by = get_pool_and_conv_props_poolLateV2(
                new_shp, self.unet_featuremap_min_edge_length, self.unet_max_numpool, current_spacing)
            print(f"[DEBUG] Recomputed pooling and convolution properties after shrinking")
            ...
            print(f"[DEBUG] Recomputed VRAM consumption: {here}")

        input_patch_size = new_shp
        print(f"[DEBUG] Final input_patch_size: {input_patch_size}")

        batch_size = Generic_UNet.DEFAULT_BATCH_SIZE_3D
        print(f"[DEBUG] Starting batch_size: {batch_size}")
        
        batch_size = int(np.floor(max(ref / here, 1) * batch_size))
        print(f"[DEBUG] Adjusted batch_size based on VRAM estimate: {batch_size}")
        
        max_batch_size = np.round(self.batch_size_covers_max_percent_of_dataset * dataset_num_voxels /
                                np.prod(input_patch_size, dtype=np.int64)).astype(int)
        max_batch_size = max(max_batch_size, self.unet_min_batch_size)
        print(f"[DEBUG] Computed max_batch_size: {max_batch_size}")
        
        batch_size = max(1, min(batch_size, max_batch_size))
        print(f"[DEBUG] Final batch_size after limiting: {batch_size}")

        do_dummy_2D_data_aug = (max(input_patch_size) / input_patch_size[0]) > self.anisotropy_threshold
        print(f"[DEBUG] do_dummy_2D_data_aug: {do_dummy_2D_data_aug}")
        ...
        print(f"[DEBUG] Created training plan: {plan}")
    ...
    def plan_experiment(self):
        ...
        print("use_nonzero_mask_for_normalization:", use_nonzero_mask_for_normalization)
        ...
        print("max_spacing_axis:", max_spacing_axis)

        remaining_axes = [i for i in range(3) if i != max_spacing_axis]
        print("remaining_axes:", remaining_axes)

        self.transpose_forward = [max_spacing_axis] + remaining_axes
        print("transpose_forward:", self.transpose_forward)

        self.transpose_backward = [np.argwhere(np.array(self.transpose_forward) == i)[0][0] for i in range(3)]
        print("transpose_backward:", self.transpose_backward)

        median_shape = np.median(np.vstack(new_shapes), 0)
        print("median_shape:", median_shape)
        ...
        print("initialized plans_per_stage:", self.plans_per_stage)

        target_spacing_transposed = np.array(target_spacing)[self.transpose_forward]
        print("target_spacing_transposed:", target_spacing_transposed)

        median_shape_transposed = np.array(median_shape)[self.transpose_forward]
        print("median_shape_transposed:", median_shape_transposed)

        print("generating configuration for 3d_fullres")
        fullres = self.get_properties_for_stage(
        ...
        self.plans_per_stage.append(fullres)
        print("plans_per_stage after fullres:", self.plans_per_stage)
        ...
        architecture_input_voxels_here = np.prod(fullres['patch_size'], dtype=np.int64)
        print("architecture_input_voxels_here:", architecture_input_voxels_here)

        if np.prod(median_shape) / architecture_input_voxels_here < self.how_much_of_a_patient_must_the_network_see_at_stage0:
            more = False
        else:
            more = True
        print("more stages needed:", more)
        ...
        if more:
            ...
            while num_voxels > self.how_much_of_a_patient_must_the_network_see_at_stage0 * architecture_input_voxels_here:
                ...
                if np.any((max_spacing / lowres_stage_spacing) > 2):
                    ...
                    print("adjusted lowres_stage_spacing (axis >2):", lowres_stage_spacing)
                else:
                    lowres_stage_spacing *= 1.01
                    print("adjusted lowres_stage_spacing:", lowres_stage_spacing)

                num_voxels = np.prod(target_spacing / lowres_stage_spacing * median_shape, dtype=np.float64)
                print("updated num_voxels:", num_voxels)

                lowres_stage_spacing_transposed = np.array(lowres_stage_spacing)[self.transpose_forward]
                print("lowres_stage_spacing_transposed:", lowres_stage_spacing_transposed)
                ...
                print("new stage properties for lowres:", new)

                architecture_input_voxels_here = np.prod(new['patch_size'], dtype=np.int64)
                print("architecture_input_voxels_here for new:", architecture_input_voxels_here)
        ...
        print("final plans dict:", plans)

        self.plans = plans
        print("assigned plans to self.plans")

        self.save_my_plans()
        print("saved plans to disk")    


    def determine_normalization_scheme(self):
        print("Entering determine_normalization_scheme")
        schemes = OrderedDict()
        print("Initialized schemes as empty OrderedDict:", schemes)
        modalities = self.dataset_properties['modalities']
        print("Retrieved modalities from dataset_properties:", modalities)
        num_modalities = len(list(modalities.keys()))
        print("Calculated num_modalities:", num_modalities)

        for i in range(num_modalities):
            modality = modalities[i]
            print(f"Processing modality index {i}, modality value: {modality}")
            if modality == "CT" or modality == 'ct':
                schemes[i] = "CT"
                print(f"Assigned schemes[{i}] = 'CT'")
            elif modality == 'noNorm':
                schemes[i] = "noNorm"
                print(f"Assigned schemes[{i}] = 'noNorm'")
            else:
                schemes[i] = "nonCT"
                print(f"Assigned schemes[{i}] = 'nonCT'")

        print("Final normalization schemes:", schemes)
        return schemes


    def save_properties_of_cropped(self, case_identifier, properties):
        filepath = join(self.folder_with_cropped_data, f"{case_identifier}.pkl")
        # print(f"Saving properties to {filepath}")
        with open(filepath, 'wb') as f:
            # print(f"Opened file for writing: {filepath}")
            pickle.dump(properties, f)
            # print(f"Properties dumped for case {case_identifier}: {properties}")
        # print(f"Finished saving properties for case {case_identifier}")
    ...
    def load_properties_of_cropped(self, case_identifier):
        ...
        with open(filepath, 'rb') as f:
            # print(f"Opened file for reading: {filepath}")
            properties = pickle.load(f)
            # print(f"Properties loaded for case {case_identifier}: {properties}")
        return properties
    ...
    def determine_whether_to_use_mask_for_norm(self):
        ...
        print("Entering determine_whether_to_use_mask_for_norm")
        modalities = self.dataset_properties['modalities']
        # print("Modalities:", modalities)
        num_modalities = len(list(modalities.keys()))
        # print("Number of modalities:", num_modalities)
        use_nonzero_mask_for_norm = OrderedDict()
        # print("Initialized use_nonzero_mask_for_norm as empty OrderedDict")

        for i in range(num_modalities):
            modality = modalities[i]
            # print(f"Processing modality {i}: {modality}")
            if "CT" in modality:
                use_nonzero_mask_for_norm[i] = False
                print(f"Modality {i} contains 'CT', set use_nonzero_mask_for_norm[{i}] = False")
            else:
                all_size_reductions = []
                # print("Collecting size reductions from dataset_properties['size_reductions'] keys:", self.dataset_properties['size_reductions'].keys())
                for k in self.dataset_properties['size_reductions'].keys():
                    reduction = self.dataset_properties['size_reductions'][k]
                    all_size_reductions.append(reduction)
                    # print(f"Appended size reduction for {k}: {reduction}")

                median_reduction = np.median(all_size_reductions)
                print("Computed median of all_size_reductions:", median_reduction)
                if median_reduction < 3 / 4.:  
                    print("Median reduction < 0.75, using nonzero mask for normalization")
                    use_nonzero_mask_for_norm[i] = True
                else:
                    print("Median reduction >= 0.75, not using nonzero mask for normalization")
                    use_nonzero_mask_for_norm[i] = False

        print("Final use_nonzero_mask_for_norm mapping:", use_nonzero_mask_for_norm)

        # print("List of cropped npz files:", self.list_of_cropped_npz_files)
        for c in self.list_of_cropped_npz_files:
            case_identifier = get_case_identifier_from_npz(c)
            # print(f"Case identifier for {c}: {case_identifier}")
            properties = self.load_properties_of_cropped(case_identifier)
            # print(f"Loaded properties for {case_identifier}: {properties}")
            properties['use_nonzero_mask_for_norm'] = use_nonzero_mask_for_norm
            # print(f"Updated properties['use_nonzero_mask_for_norm'] for {case_identifier}:", properties)
            self.save_properties_of_cropped(case_identifier, properties)
            print(f"Saved updated properties for {case_identifier}")

        use_nonzero_mask_for_normalization = use_nonzero_mask_for_norm
        # print("Returning use_nonzero_mask_for_normalization:", use_nonzero_mask_for_normalization)
    ...
    def write_normalization_scheme_to_patients(self):
        ...
        :return: 
        """    
        print("Entering write_normalization_scheme_to_patients")
        print("List of cropped npz files:", self.list_of_cropped_npz_files)
        for c in self.list_of_cropped_npz_files:
            ...
            print(f"Set properties['use_nonzero_mask_for_norm'] to {self.plans['use_mask_for_norm']}")
            self.save_properties_of_cropped(case_identifier, properties)
            print(f"Saved updated properties for {case_identifier}")
        print("Completed write_normalization_scheme_to_patients")


    def run_preprocessing(self, num_threads):
        print("Entering run_preprocessing with num_threads:", num_threads)
        gt_dir = join(self.preprocessed_output_folder, "gt_segmentations")
        print(f"Target gt_seg output dir: {gt_dir}")
        if os.path.isdir(gt_dir):
            print(f"Directory exists, removing: {gt_dir}")
            shutil.rmtree(gt_dir)
            print(f"Removed directory {gt_dir}")
        src_gt = join(self.folder_with_cropped_data, "gt_segmentations")
        # print(f"Copying gt_segmentations from {src_gt} to {gt_dir}")
        shutil.copytree(src_gt, gt_dir)
        # print("Completed copying gt_segmentations")

        normalization_schemes = self.plans['normalization_schemes']
        print("Loaded normalization_schemes:", normalization_schemes)
        use_nonzero_mask_for_normalization = self.plans['use_mask_for_norm']
        print("Loaded use_nonzero_mask_for_normalization:", use_nonzero_mask_for_normalization)
        intensityproperties = self.plans['dataset_properties']['intensityproperties']
        print("Loaded intensityproperties:", intensityproperties)
        ...
        print(f"Found preprocessor_class: {preprocessor_class}")
        assert preprocessor_class is not None, "Preprocessor class not found"
        print("Assertion passed: preprocessor_class is not None")
        ...
        print(f"Instantiated preprocessor: {preprocessor}")

        target_spacings = [i["current_spacing"] for i in self.plans_per_stage.values()]
        print("Calculated target_spacings for each stage:", target_spacings)

        if self.plans['num_stages'] > 1 and not isinstance(num_threads, (list, tuple)):
            print("Multiple stages and num_threads is not tuple/list, converting to tuple")
            num_threads = (default_num_threads, num_threads)
        elif self.plans['num_stages'] == 1 and isinstance(num_threads, (list, tuple)):
            print("Single stage but num_threads is tuple/list, selecting last element")
            num_threads = num_threads[-1]
        print("Final num_threads to use:", num_threads)

        print("Running preprocessor.run()")
        preprocessor.run(
            target_spacings,
        ...
        print("Completed run_preprocessing")
